...
local strings = {
    en = {
        ...
        commands = {
            ...
            clipboard = "Open platform-aware clipboard and SSH copy help",
        ...
        help = {
            ...
            map_scroll_down = "Scroll Clarity help down",
            map_scroll_up = "Scroll Clarity help up",
            navigation_line = "Navigate: `j`/`k` or `Ctrl-d`/`Ctrl-u` scroll; `q` or `Esc` closes.",
            open_failed = "Clarity help could not open. Your first-start guide remains pending; run `:ClarityStart` to retry.",
            action_failed = "That Clarity action could not finish. Reopen `:ClarityStart` and try the recovery path.",
            ...
            start_recovery_clipboard = "- `c` Open platform-aware clipboard help",
            ...
            clipboard_paths_header = "## Copy and paste paths",
            clipboard_path_1 = "1. Copy from Neovim",
            clipboard_path_1_detail = '   Use normal yank with `unnamedplus`, or `"+y` explicitly. SSH OSC52 supports this outbound copy path.',
            clipboard_path_2 = "2. Paste into a terminal session",
            clipboard_path_2_detail = "   Use your terminal paste shortcut (`Ctrl + Shift + V`, or `Cmd + V` on macOS).",
            clipboard_path_3 = "3. OSC52 boundary",
            clipboard_path_3_detail = "   Clarity promises OSC52 copy only. It does not promise remote clipboard read or paste.",
            ...
            clipboard_config_line_2 = "- Audit reports desktop, WSL, SSH OSC52, or missing provider without reading clipboard contents.",
            clipboard_rule_header = "## Session-aware rule",
            clipboard_rule_line_1 = "- Desktop and WSL use their detected native provider.",
            clipboard_rule_line_2 = "- Plain SSH without a desktop display uses OSC52 for copy unless you set a provider yourself.",
            clipboard_rule_line_3 = "- With no provider, editing remains core-ready; only the clipboard profile is degraded.",
        ...
        keymaps = {
            ...
            toggle_fold = "Toggle current code fold",
        ...
        notifications = {
            ...
            fold_failed = "Clarity could not toggle this fold. Run `:messages` for details.",
    ...
    zh = {
        ...
        commands = {
            ...
            clipboard = "打开平台感知的剪贴板与 SSH 复制帮助",
        ...
        help = {
            ...
            map_scroll_down = "向下滚动 Clarity 帮助",
            map_scroll_up = "向上滚动 Clarity 帮助",
            navigation_line = "导航：用 `j`/`k` 或 `Ctrl-d`/`Ctrl-u` 滚动；用 `q` 或 `Esc` 关闭。",
            open_failed = "Clarity 帮助无法打开。首次引导仍处于待显示状态；请运行 `:ClarityStart` 重试。",
            action_failed = "该 Clarity 操作未能完成。请重新打开 `:ClarityStart`，并尝试恢复路径。",
            ...
            start_recovery_clipboard = "- `c` 打开平台感知的剪贴板帮助",
            ...
            clipboard_paths_header = "## 复制与粘贴路径",
            clipboard_path_1 = "1. 从 Neovim 复制",
            clipboard_path_1_detail = '   使用启用 `unnamedplus` 的普通 yank，或显式使用 `"+y`。SSH OSC52 支持这条向外复制路径。',
            clipboard_path_2 = "2. 粘贴到终端会话",
            clipboard_path_2_detail = "   使用终端的粘贴快捷键（`Ctrl + Shift + V`，macOS 使用 `Cmd + V`）。",
            clipboard_path_3 = "3. OSC52 边界",
            clipboard_path_3_detail = "   Clarity 只承诺 OSC52 复制，不承诺远程读取剪贴板或粘贴。",
            clipboard_config_header = "## 这套配置当前的行为",
            clipboard_config_line_1 = "- 已启用 `clipboard=unnamedplus`，所以当 provider 正常时，普通 yank 通常就会进入系统剪贴板。",
            clipboard_config_line_2 = "- 审计只报告桌面、WSL、SSH OSC52 或缺少 provider，不会读取剪贴板内容。",
            clipboard_rule_header = "## 会话感知规则",
            clipboard_rule_line_1 = "- 桌面与 WSL 使用检测到的原生 provider。",
            clipboard_rule_line_2 = "- 没有桌面显示的纯 SSH 会话会使用 OSC52 复制，除非你已自行设置 provider。",
            clipboard_rule_line_3 = "- 没有 provider 时，核心编辑仍然就绪；只有剪贴板 profile 降级。",
        ...
        notifications = {
            ...
            fold_failed = "Clarity 无法切换此折叠。请运行 `:messages` 查看详情。",
